...
class FrameVis(Visualizer):
    ...
    def draw_bb3d_edge(self, frame, p1, p2, v1, v2, color):
        ...
        if v1 > 0 and v2 > 0:
            ...
            tp1 = tuple(p1.astype(int))
            tp2 = tuple(p2.astype(int))
...
class MapVis(Visualizer):
    ...
    def draw_camera_FOV_on_map(self, map, camera_model, map_model, roi,
        ...
        cv2.line(map, tuple(r1.astype(int)), tuple(r2.astype(int)), color, thickness)
        cv2.line(map, tuple(r2.astype(int)), tuple(r3.astype(int)), color, thickness)
        cv2.line(map, tuple(r3.astype(int)), tuple(r4.astype(int)), color, thickness)
        cv2.line(map, tuple(r4.astype(int)), tuple(r1.astype(int)), color, thickness)
...
#                 print(frame_id, np.linalg.norm(vel))
    ...
    def draw_vehicle_states_on_global_map(self, map, map_model, vehicles, frame_inverval):
        ...
        for vid, vehicle in vehicles.items():
            ...
            cv2.line(map, tuple(r1.astype(int)), tuple(r2.astype(int)), color, thickness)
            cv2.line(map, tuple(r1.astype(int)), tuple(r3.astype(int)), color, thickness)
            cv2.line(map, tuple(r2.astype(int)), tuple(r4.astype(int)), color, thickness)
            cv2.line(map, tuple(r3.astype(int)), tuple(r4.astype(int)), color, thickness)
