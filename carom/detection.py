...
import carom.transforms as TT
import carom.utils as utils
from carom.background import CameraShakeRectifier
#from carom.engine import train_one_epoch
...
def compute_contour_from_mask(mask):
    ...
    #print("contours from compute contour from mask", contours)
...
class DetectorTorchvisionMaskRCNN(Detector):
    ...
    def __init__(self, folder, prefix, postfix, pretrained=False):
        ...
        if not pretrained:
            model.load_state_dict(torch.load('/home/nithish/autoware/src/universe/autoware.universe/perception/carom/carom/retrained_net.pth'))
    ...
    def detect(self, frame_id, frame):
        ...
        '''
        cv2.imshow("Raw mask", masks_raw)
        print("masks_raw: ", masks_raw)
        '''
        masks_raw = masks_raw > 0.5
        #print("Checking whether there is anything above the threshold: ",np.any(masks_raw))
        '''
        masks_raw = masks_raw.astype(np.uint8) * 255
        cv2.imshow("Raw mask after threshold", masks_raw)
        '''
        

        #print("scores_raw, labels_raw: ", scores_raw, labels_raw)
        #print("boxes_raw, masks_raw: ", boxes_raw.shape, masks_raw.shape)
        ...
        # This modification is very important
        # when there is only one vehicle in the field of view of the camera n==1
        # when n==1 the mask shape is (540, 896) which is the same as the input image and masks_raw[i] is similar to accessing
        # the first row of the masks_raw rather than accessing the first entire image of mask_raw as there is a missing dimension
        # As a result the masks after assigned with masks_raw (i.e. masks[j] = masks_raw[i]) will be completely black always.

        # This change is critical and has to be upated in the author's code. Raise an issue and provide the update through the github repo.

        if n==1:
            boxes[0] = boxes_raw[0]
            masks[0] = masks_raw
            #print("Checking whether there is anything above the threshold (Inside if condition): ",np.any(masks[0]))
            labels[0] = labels_raw[0]
            scores[0] = scores_raw[0]
            ...
            for j in range(n):
                ...
                boxes[j] = boxes_raw[i]
                masks[j] = masks_raw[i]
                #print("Checking whether there is anything above the threshold (Inside else condition): ",np.any(masks[j]))
        ...
        for i in range(n):
            ...
            #print("center of the box (MaskRCNN): ", centers)
            #print("Just before calling compute_contour_from_mask", masks[i])
            #print("Checking whether there is anything above the threshold: ",np.any(masks[i]))
            contour = compute_contour_from_mask(masks[i])
            #print("contour from mask (MaskRCNN): ", contour)
            contours.append(contour)

        #print("contours from MaskRCNN: ", contours)
...
def test_detector_fine_tuning():
    ...
    torch.save(model.state_dict(), '/home/nithish/autoware/src/universe/autoware.universe/perception/carom/carom/retrained_net.pth')
    model.eval()
    # model._save_to_state_dict('/home/nithish/autoware/src/universe/autoware.universe/perception/carom/carom/retrained_net.pth')
