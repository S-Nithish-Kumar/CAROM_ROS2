...
class Tracker(object):
    ...
    def track_iteration_online(self, frame_id, frame_current):
        ...
        if camera_image_subscriber.euclidean_distance_between_rsu_and_ego_vehicle<80.0:
            #print("RSU and Ego vehicle are closer")
...
class CameraImageSubscriber(Node):
    def __init__(self):
        ...
        self.euclidean_distance_between_rsu_and_ego_vehicle = 0.0

    def frame_transform_rsu_frame_to_base_link(self):
        try:
            t1 = self.tf_buffer.lookup_transform('rsu_frame', 'base_link', self.time_stamp)
            self.x_tf, self.y_tf = t1.transform.translation.x, t1.transform.translation.y
            self.euclidean_distance_between_rsu_and_ego_vehicle = (self.x_tf**2 + self.y_tf**2)**0.5
            #print("Euclidean_distance_between_rsu_and_ego_vehicle: ", self.euclidean_distance_between_rsu_and_ego_vehicle)
        except TransformException as ex:
            self.get_logger().info(f'Could not transform "rsu_frame" to "base_link": {ex}')
            return
    
    def display_image(self, camera_msg):
        self.header_info = camera_msg.header
        self.time_stamp = camera_msg.header.stamp
        self.frame_transform_rsu_frame_to_base_link()
