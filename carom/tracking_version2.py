...
from carom.background import CameraShakeRectifier
from carom.background import KLTOpticalFlowTracker
from carom.boundingbox3D import BB3D
from carom.boundingbox3D import bb3d_perspective
from carom.calibration import Camera2DGroundModel
from carom.calibration import MapModel
from carom.detection import DetectorTorchvisionMaskRCNN
from carom.detection import DetectorWithSavedResults
from carom.detection import calculate_box_size
from carom.detection import calculate_mask_overlap
from carom.detection import check_box_bottom_occlusion
from carom.detection import check_box_on_mask_bg
from carom.detection import check_box_overlap
from carom.detection import compute_center_from_box
from carom.detection import compute_contour_from_mask
from carom.visualization import FrameVis
from carom.visualization import MapVis
...
class Vehicle(object):
    ...
    def state_init(self, instance, states_3d):
        ...
        # initial pos. Nithish added this.
        self.custom_pos = pos
    ...
    def state_update(self, instance, states_3d):
        ...
        if vel_n > 0.5:
            self.heading = states_3d.heading
            # checked. Heading before and after averaging is the same. So do this step just leave it commented as is.
            #print("heading before averaging: ", self.heading)
            #self.heading = (self.heading + states_3d.heading) / 2
            #print("heading after averaging: ", self.heading)
        ...
        #print("self.ss[0:2, 0]: (i.e. updates pos): ", self.ss[0:2, 0])

        self.custom_pos = [self.ss[0, 0], self.ss[1, 0], 0.0]
        #print("updated pos: ", self.custom_pos)
...
class Tracker(object):
    ...
    def track_iteration_online(self, frame_id, frame_current):
        ...
        #print("instances from MaskRCNN: ", instances)
        ...
        instances_3d_states = self.compute_3d_states(
            self.camera_model, self.map_model, instances)
        ...
        #print("dim : ", self.instances_3d_states[0].dim)
        #print("dim_uc : ", self.instances_3d_states[0].dim_uc)
        publish_3d_states(self.current_vehicles)

        #print("Heading: ", self.instances_3d_states[0].heading)
        #print("Dimension: ", self.instances_3d_states[0].dim)
        #print("Position: ", self.instances_3d_states[0].pos)
    ...
    def detect_instances(self, frame_id, frame):
        ...
        n = boxes.shape[0]
        for i in range(n):
            #print("labels before filtering:", labels[i])
            if np.sum(masks[i]) <= 50:
                #print("Low MASK threshold")
                continue

            if scores[i] < score_th:
                #print("Low SCORE threshold for label: ", labels[i])
                continue

            if not check_box_on_mask_bg(boxes[i], self.mask_bg):
                #print("Check box on mask bg FAIL")
                continue
    ...
    def check_label(self, label):
        ...
        '''
        #print("label: ", label)    # checked. Is always 3.
    ...
    def compute_3d_states(self, camera_model, map_model, instances):
        '''
        ...
        for i, instance in enumerate(instances):
            ...
            bb3d = bb3d_perspective(camera_model, instance.contour,
    ...
    def estimate_vehicle_states(self, frame_id, instances, instances_states_3d):
        ...
        #print("Right before state prediction step")
        #print("What's in self.current_vehicles: ", self.current_vehicles.items())
        for vid, vehicle in self.current_vehicles.items():
            #print("Entering state prediction step")
        ...
        #for vid, vehicle in self.current_vehicles.items():
            #print(self.current_vehicles[vid].custom_pos)
    ...
    def load_data_file(self, fn, fmt, is_integer):
        ...
        with warnings.catch_warnings():
            ...
            if fmt == 'csv':
                if is_integer:
                    array = np.loadtxt(fn + '.csv', delimiter=',', dtype=int)
                else:
    ...
    def save_instance_instance_association(self, conf, frame_id,
        ...
        array_c2p = np.zeros((n_c2p, 2), dtype=int)
        array_p2c = np.zeros((n_p2c, 2), dtype=int)
    ...
    def save_vehicle_instance_association(self, conf, frame_id,
        ...
        array_v2i = np.zeros((n, 2), dtype=int)
        array_i2v = np.zeros((m, 2), dtype=int)
...
def init_pipeline(conf, init_frame_id, need_rectification, replay, extension):
    ...
    tracker = Tracker(map_model, klt_tracker, detector, camera_model,
                      ...
                      h_margin=15, v_margin=10, fps=10)
    ...
    return tracker, csr, map_local, map_global
...
def pipeline_multiple_tracker_association(trackers):
    ...
    for i, tracker_i in enumerate(trackers):
        ...
        n_i = len(vehicles_i)
        association_i = np.zeros((n_i, m), dtype=int)
...
def pipeline_visualize(tracker, frame_rect, frame_id, fvis, mvis, rot_map, h_line, writer, render=False):
...
def test_tracker_init(cam_id, track_id, folder, prefixes):
    # replay = True
    replay = False
    ...
    tracker, csr, map_local, map_gmap \
        = init_pipeline(conf, init_frame_id, need_rectification, replay, extension=extension)
    
    #print("CAP_PROP_FRAME_WIDTH", cap.get(cv2.CAP_PROP_FPS))
    #print("frame shape:", first_frame.shape[2])
    ...
    return replay, save, save_video, render, extension, fmt, need_rectification, direction, postfix, prefix, h_lines, h_line, conf,\
        fvis, mvis, colors, init_frame_id, tracker, csr, map_local, map_gmap, writer

def test_tracker(cam_id, track_id, folder, prefixes, custom_frame_id, camera_frame, replay, save, save_video, \
        render, extension, fmt, need_rectification, direction, postfix, prefix, h_lines, h_line, conf,\
        fvis, mvis, colors, init_frame_id, tracker, csr, map_local, map_gmap, writer):
    
    if custom_frame_id == 0:
        # Process the first frame.
        first_frame = camera_frame
        if replay:
...
def heading_vector_to_angle(heading_vector):
    vx, vy = heading_vector[0], heading_vector[1]
    yaw_radians = np.arctan2(vy, vx)
    #print("Yaw angle: ", yaw_radians)
    #yaw_degrees = np.degrees(yaw_radians) + 180.0
...
def publish_3d_states(current_vehicles):
    
    # calculate the time difference between input and the output including ros2 msg conversion time.
    end_time = time.time()
    
    #camera_image_subscriber.csvwriter.writerow([round(time_difference * 1000, 0)])

    # calculate the time taken for ros2 msg conversion
    #ros_msg_conversion_start_time = time.time()    

    publish_objects_msg = DetectedObjects()
    #for vid, vehicle in self.current_vehicles.items():
        #print(self.current_vehicles[vid].custom_pos)
    objects = [0 for o in range(len(current_vehicles))]
    classification = [0]
    n = 0
    for vid, vehicle in current_vehicles.items():
        objects[n] = DetectedObject()
        if current_vehicles[vid].heading is not None:
            #pos, vel, pos_flag = vehicle.get_pos_vel()
            self_pos, self_center, _hp, (qx, qy, qz), (self_nv, _nt, _nu) = vehicle.get_3D_anchor_points()
            #self_center = vehicle.get_center()
            classification[0] = ObjectClassification()
            objects[n].existence_probability = 1.0
            classification[0].label = 1
            # This if condition is added cuz when the vehicle comes near the intersection its length increases and its no more the length of a car
            # If the label is still going to be 1 then the tracking fails. So make it 3 which means bus and the tracking works properly.
            if current_vehicles[vid].dim[0] > 5:
                classification[0].label = 3
            objects[n].classification = list(classification)
            objects[n].kinematics.pose_with_covariance.pose.position.x = self_center[0]
            objects[n].kinematics.pose_with_covariance.pose.position.y = self_center[1]
            objects[n].kinematics.pose_with_covariance.pose.position.z = current_vehicles[vid].dim[2]/2
            yaw_angle = heading_vector_to_angle(current_vehicles[vid].heading)
            #yaw_angle = heading_vector_to_angle(_hp)
            quaternion_vector = euler_to_quaternion(0, 0, yaw_angle)
            objects[n].kinematics.pose_with_covariance.pose.orientation.x = quaternion_vector[0]
            objects[n].kinematics.pose_with_covariance.pose.orientation.y = quaternion_vector[1]
            objects[n].kinematics.pose_with_covariance.pose.orientation.z = quaternion_vector[2]
            objects[n].kinematics.pose_with_covariance.pose.orientation.w = quaternion_vector[3]
            objects[n].kinematics.orientation_availability = 1
            objects[n].shape.dimensions.x = current_vehicles[vid].dim[0]
            objects[n].shape.dimensions.y = current_vehicles[vid].dim[1]
            objects[n].shape.dimensions.z = current_vehicles[vid].dim[2]
            n = n+1

            '''
            camera_image_subscriber.csvwriter.writerow([pos[0], pos[1], pos[2], \
                    quaternion_vector[0], quaternion_vector[1], quaternion_vector[2], quaternion_vector[3], \
                    camera_image_subscriber.ego_vehicle_ground_truth_px, \
                    camera_image_subscriber.ego_vehicle_ground_truth_py, \
                    camera_image_subscriber.ego_vehicle_ground_truth_pz, \
                    camera_image_subscriber.ego_vehicle_ground_truth_qx, \
                    camera_image_subscriber.ego_vehicle_ground_truth_qy, \
                    camera_image_subscriber.ego_vehicle_ground_truth_qz, \
                    camera_image_subscriber.ego_vehicle_ground_truth_qw])
            '''

    '''
    groundtruth_object_msg = DetectedObjects()
    groundtruth_object = [0]
    groundtruth_object[0] = DetectedObject()
    groundtruth_object_classification = [0]
    groundtruth_object_classification[0] = ObjectClassification()
    groundtruth_object[0].existence_probability = 1.0
    groundtruth_object_classification[0].label = 1
    groundtruth_object[0].classification = list(groundtruth_object_classification)
    groundtruth_object[0].kinematics.pose_with_covariance.pose.position.x = camera_image_subscriber.ego_vehicle_ground_truth_px
    groundtruth_object[0].kinematics.pose_with_covariance.pose.position.y = camera_image_subscriber.ego_vehicle_ground_truth_py
    groundtruth_object[0].kinematics.pose_with_covariance.pose.position.z = camera_image_subscriber.ego_vehicle_ground_truth_pz
    groundtruth_object[0].kinematics.pose_with_covariance.pose.orientation.x = camera_image_subscriber.ego_vehicle_ground_truth_qx
    groundtruth_object[0].kinematics.pose_with_covariance.pose.orientation.y = camera_image_subscriber.ego_vehicle_ground_truth_qy
    groundtruth_object[0].kinematics.pose_with_covariance.pose.orientation.z = camera_image_subscriber.ego_vehicle_ground_truth_qz
    groundtruth_object[0].kinematics.pose_with_covariance.pose.orientation.w = camera_image_subscriber.ego_vehicle_ground_truth_qw
    groundtruth_object[0].kinematics.orientation_availability = 1
    # dimensions of Mercedes Coupe 2020
    groundtruth_object[0].shape.dimensions.x = 4.67
    groundtruth_object[0].shape.dimensions.y = 1.81
    groundtruth_object[0].shape.dimensions.z = 1.44
    '''


    publish_objects_msg.objects = objects  
    publish_objects_msg.header = camera_image_subscriber.header_info
    publish_objects_msg.header.frame_id = "camera_infrastructure_local_frame"
    #publish_objects_msg.header.frame_id = "base_link"

    # calculate the time difference between input and the output including ros2 msg conversion time.
    time_difference = end_time - camera_image_subscriber.start_time
    #print("Time difference (milli sec) between input and output: ", time_difference * 1000)
    #print("Time difference (rounded up and in milli sec) between input and output: ", round(time_difference * 1000, 0))
    process_time_msg = Float32()
    process_time_msg.data = round(time_difference * 1000, 0)
    camera_image_subscriber.process_time_publisher.publish(process_time_msg)

    # calculate the time taken for ros2 msg conversion
    '''
    time_difference = time.time() - ros_msg_conversion_start_time
    print("Time taken (milli sec) for ros2 msg conversion: ", time_difference * 1000)
    print("Time taken (rounded up and in milli sec) for ros2 msg conversion: ", round(time_difference * 1000, 0))
    '''

    camera_image_subscriber.infrastructure_objects_publisher.publish(publish_objects_msg)

    # Publish ground truth object for validation with the prediction results.
    '''
    groundtruth_object_msg.objects = groundtruth_object
    groundtruth_object_msg.header = camera_image_subscriber.header_info
    groundtruth_object_msg.header.frame_id = "map"
    camera_image_subscriber.groundtruth_object_publisher.publish(groundtruth_object_msg)
    '''

def handle_signal(signal, frame):
    #print("Interrupt received, closing the file.")
    print("Signal: ", signal) # returns the signal id which is useful to find out what signal caused this signal handle.
    print("Frame: ", frame) # provides exact location (i.e code line number and code location) when interrupted.
    #camera_image_subscriber.csvfile.close()
    sys.exit(0) # exits the python interpreter. 0 usually means successful execution. Any value other than 0 means there is an error.
...
class CameraImageSubscriber(Node):
    def __init__(self):
        ...
        self.image_subscription = self.create_subscription(Image, "/carla/camera_infrastructure/rate_downsampled/image", self.display_image, qos_profile)
        self.image_subscription
        self.br = CvBridge()

        self.infrastructure_objects_publisher = self.create_publisher(DetectedObjects, \
                '/perception/object_recognition/detection/carom/objects', 10)
        self.header_info = Header()

        # For validation with the prediction results
        self.groundtruth_object_publisher = self.create_publisher(DetectedObjects, \
                '/perception/groundtruth_object', 10)

        self.prefixes = ['carla_eastbound_res_896x504', 'carla_eastbound', 'westbound', 'eastbound', 'northbound', 'southbound']
        self.folder = '/home/nithish/autoware/src/universe/autoware.universe/perception/carom/carom/data'
        self.video_id = 0
        self.prefix_id = 0

        self.custom_frame_id = 0

        self.start_time = 0
        self.csvfile = open('/home/nithish/monocular_object_detection/CAROM/data_analysis/carla_eastbound_res_896x504_with_ros_topic_from_video_and_just_ego_vehicle.csv', 'a', newline='')
        self.csvwriter = csv.writer(self.csvfile)

        # To record the pose of the ego vehicle for comparison with CAROM's detection for delay computation
        ...
        self.ox, self.oy, self.oz, self.ow = 0, 0, 0, 0
        self.predicted_displacement_x, self.predicted_displacement_y = 0, 0

        self.tf_header_stamp = Time()
        self.ego_vehicle_ground_truth_px, self.ego_vehicle_ground_truth_px_copy = 0.0, 0.0
        self.ego_vehicle_ground_truth_py, self.ego_vehicle_ground_truth_py_copy = 0.0, 0.0
        self.ego_vehicle_ground_truth_pz, self.ego_vehicle_ground_truth_pz_copy = 0.0, 0.0
        self.ego_vehicle_ground_truth_qx, self.ego_vehicle_ground_truth_qx_copy = 0.0, 0.0
        self.ego_vehicle_ground_truth_qy, self.ego_vehicle_ground_truth_qy_copy = 0.0, 0.0
        self.ego_vehicle_ground_truth_qz, self.ego_vehicle_ground_truth_qz_copy = 0.0, 0.0
        self.ego_vehicle_ground_truth_qw, self.ego_vehicle_ground_truth_qw_copy = 0.0, 0.0

        self.tf_broadcaster = TransformBroadcaster(self)

        self.process_time_publisher = self.create_publisher(Float32, '/perception/object_recognition/detection/carom/objects/delay', 10)

        self.tf_buffer = Buffer()
        self.tf_listener = TransformListener(self.tf_buffer, self)

        self.timer = self.create_timer(0.01, self.frame_transformation)
        self.timer1 = self.create_timer(0.01, self.dummy_pose_broadcaster)
        self.timer2 = self.create_timer(0.01, self.frame_transformation2)
    ...
    def dummy_pose_broadcaster(self):
        t = TransformStamped()

        t.header.stamp = self.tf_header_stamp
        t.header.frame_id = 'map'
        t.child_frame_id = 'dummy_base_link'

        t.transform.translation.x = self.ego_vehicle_ground_truth_px_copy
        t.transform.translation.y = self.ego_vehicle_ground_truth_py_copy
        t.transform.translation.z = self.ego_vehicle_ground_truth_pz_copy

        t.transform.rotation.x = self.ego_vehicle_ground_truth_qx_copy
        t.transform.rotation.y = self.ego_vehicle_ground_truth_qy_copy
        t.transform.rotation.z = self.ego_vehicle_ground_truth_qz_copy
        t.transform.rotation.w = self.ego_vehicle_ground_truth_qw_copy

        self.tf_broadcaster.sendTransform(t)

    
    def display_image(self, camera_msg):
        self.header_info = camera_msg.header
        self.camera_frame_BGRA = self.br.imgmsg_to_cv2(camera_msg)
        self.camera_frame = cv2.cvtColor(self.camera_frame_BGRA, cv2.COLOR_BGRA2BGR)
        #print("frame shape from carla: ", self.camera_frame.size)

        self.ox_copy, self.oy_copy, self.oz_copy, self.ow_copy = self.ox, self.oy, self.oz, self.ow
        self.x_tf_copy, self.y_tf_copy, self.z_tf_copy = self.x_tf, self.y_tf, self.z_tf
        self.yaw_tf_copy = self.yaw_tf

        self.ego_vehicle_ground_truth_px_copy = self.ego_vehicle_ground_truth_px
        self.ego_vehicle_ground_truth_py_copy = self.ego_vehicle_ground_truth_py
        self.ego_vehicle_ground_truth_pz_copy = self.ego_vehicle_ground_truth_pz
        self.ego_vehicle_ground_truth_qx_copy = self.ego_vehicle_ground_truth_qx
        self.ego_vehicle_ground_truth_qy_copy = self.ego_vehicle_ground_truth_qy
        self.ego_vehicle_ground_truth_qz_copy = self.ego_vehicle_ground_truth_qz 
        self.ego_vehicle_ground_truth_qw_copy = self.ego_vehicle_ground_truth_qw

        #self.dummy_pose_broadcaster()

        
        # calculate the time difference between the input and the output starting from frame one.
        if self.custom_frame_id>0:
            self.start_time = time.time()
        

        if (self.camera_frame is not None or self.camera_frame != 0) and self.custom_frame_id == 0:
            self.replay, self.save, self.save_video, self.render, self.extension, self.fmt, self.need_rectification, self.direction, \
            self.postfix, self.prefix, self.h_lines, self.h_line, self.conf, self.fvis, self.mvis, self.colors, self.init_frame_id, \
            self.tracker, self.csr, self.map_local, self.map_gmap, self.writer = \
            test_tracker_init(self.prefix_id, self.video_id, self.folder, self.prefixes)
        test_tracker(self.prefix_id, self.video_id, self.folder, self.prefixes, self.custom_frame_id, self.camera_frame, \
            self.replay, self.save, self.save_video, self.render, self.extension, self.fmt, self.need_rectification, self.direction, \
            self.postfix, self.prefix, self.h_lines, self.h_line, self.conf, self.fvis, self.mvis, self.colors, self.init_frame_id, \
            self.tracker, self.csr, self.map_local, self.map_gmap, self.writer)
        self.custom_frame_id = self.custom_frame_id + 1
        #cv2.imshow('image', self.camera_frame)
        if cv2.waitKey(10) & 0xFF == ord('q'):
            exit()

    def odometry_filter(self, odometry_msg):
        self.ego_vehicle_ground_truth_px = odometry_msg.pose.pose.position.x
        self.ego_vehicle_ground_truth_py = odometry_msg.pose.pose.position.y
        self.ego_vehicle_ground_truth_pz = odometry_msg.pose.pose.position.z
        self.ego_vehicle_ground_truth_qx = odometry_msg.pose.pose.orientation.x
        self.ego_vehicle_ground_truth_qy = odometry_msg.pose.pose.orientation.y
        self.ego_vehicle_ground_truth_qz = odometry_msg.pose.pose.orientation.z
        self.ego_vehicle_ground_truth_qw = odometry_msg.pose.pose.orientation.w
    ...
    def frame_transformation(self):
        try:
            ...
            self.tf_header_stamp = t.header.stamp
        except TransformException as ex:
            self.get_logger().info(f'Could not transform "base_link" to "camera_infrastructure_local_frame": {ex}')
            return 0
        
    def frame_transformation2(self):
        try:
            t = self.tf_buffer.lookup_transform('dummy_base_link', 'camera_infrastructure_local_frame', rclpy.time.Time())
            ...
            self.get_logger().info(f'Could not transform "dummy_base_link" to "camera_infrastructure_local_frame": {ex}')
            return 0
    ...
    def infrastructure_objects_transformer(self, infrastructure_detection_msg): 
        ...
        infrastructure_detection_msg.header.frame_id = "dummy_base_link"
...
def main(args=None):
    ...
    global camera_image_subscriber 
